...
import { isEmpty } from 'lodash';
  ...
  let chart = null;
  const salesAchiev = () => {
    const ctx = document.getElementById('sales-achiev-chart');
    if (!ctx) return;
    if (chart != null) {
        chart.destroy();
    }
    const year = $("#achiev-search").val();
  
    fetch(`/dashboard/salesachiev?year=${year}`)
      .then(a => {
        return a.json();
      })
      .then(result => {
          ...
          options: {
            ...
            plugins: {
              ...
              htmlLegend: {
                ...
                containerID: 'sales-achiev-legend',
              },
              tooltip: {
                callbacks: {
