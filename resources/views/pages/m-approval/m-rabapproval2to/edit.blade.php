    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">
                    Master RAB Approval 2 To - Edit 🗄️
                </h1>
            </div>
        </div>
        ...
        <div class="flex flex-row text-xs">
            <label class="flex flex-row text-xs">
                ...
                {{-- <a href="{{route('rabapproval3to.editpage')}}" class="btn bg-indigo-500 hover:bg-indigo-600 text-white text-xs mb-3 ml-5">
                    Master RAB Approval 3 To - Edit
                </a> --}}
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#approvalto').on("click", ".btn-modal", function () {
                ...
                $.ajax({
                    type: "GET",
                    url: `/data-master/m-rabapproval2to/getdata/${id}`,
                    success: function (response) {
                        const csrf_token = $('meta[name="csrf-token"]').attr('content');

                        $(".modal-content").html(`
                            <form method="post" class="type_update" enctype="multipart/form-data" action="/data-master/m-rabapproval2to/update/${id}">
