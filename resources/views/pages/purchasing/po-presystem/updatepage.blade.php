<x-app-layout background="bg-white">
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <!-- Page header -->
        <div class="mb-8">
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">PO Pre System Edit 📝</h1>
        </div>
        @if ($dataPO->file != null)
    ...
    <div class="px-5 py-4">
        <div class="space-y-3">
            <form method="post" enctype="multipart/form-data" action="{{ route('po-presystem.update', ['idPO' => $dataPO->idrec]) }}" id="myForm">
                @csrf
                ...
                <div class="flex justify-between flex-col md:flex-row">
                    <label class="text-sm font-medium mb-1"
                        ...
                        <select id="company" name="company"
                            class="company form-select w-full md:w-3/4 px-2 py-1" hidden required>
                            <option selected hidden value="">Select Company </option>
                            @foreach ( $dataChildCompany as $company )
                                <option value="{{$company->id_company}}" {{$dataPO->id_company == $company->id_company ? 'selected':''}}>{{$company->name}}</option>
                            @endforeach
                        </select>
                        <input id="companyTest" name="companyTest" class="companyTest form-input w-full md:w-3/4 px-2 py-1 read-only:bg-slate-200" value="{{$dataPO->companyName}}" readonly/>
                </div>
                <div class="flex md:flex-row mt-3 mb-3">
                    <label class="block w-1/4 text-sm"></label>
                    <button type="button" class="btn btn-sm bg-indigo-500 hover:bg-indigo-600 text-white mr-5" id="clearCompany">Change Company</button>
                </div>
                @else
                <div class="flex justify-between flex-col md:flex-row">
                    <label class="text-sm font-medium mb-1"
                        ...
                        value="{{$dataPO->id_company}}" readonly hidden/>
                </div>
                @endif
                <div class="flex justify-between flex-col md:flex-row mt-3">
                    <label class="text-sm font-medium mb-1" for="date_po">PO Date<span
                        class="text-rose-500">*</span></label>
                    <input id="date_po" name="date_po" value="{{date('Y-m-d', strtotime($dataPO->date_po))}}"
                ...
                <div class="flex justify-between flex-col md:flex-row mt-3">
                    <label class="text-sm font-medium mb-1" for="due_date">Due Date<span
                        class="text-rose-500">*</span></label>
                    <input id="due_date" name="due_date" value="{{date('Y-m-d', strtotime($dataPO->due_date))}}"
                ...
                <div class="flex flex-col md:flex-row mt-3">
                    ...
                    <input class="idemployee form-input w-full md:w-1/2 px-2 py-1 read-only:bg-slate-200" id="idemployee" name="idemployee" value="{{$dataPO->idemployee}}" hidden readonly/>
                    <input class="employee1 form-input w-full md:w-1/2 px-2 py-1 read-only:bg-slate-200" id="employee1" name="employee1" value="{{$dataPO->first_name}} {{$dataPO->last_name}}" readonly/>
                    @if (Auth::user()->company_id == '0' || Auth::user()->company_id == '999' || Auth::user()->company_id == '888')
                ...
                <div class="flex flex-row mb-3 mt-3">
                    <label class="block w-full md:w-1/4 text-sm font-medium mt-1">PO # / Invoice # / RAB #<span class="text-rose-500">*</span></label>
                        <div style="width: 20.8rem; margin-right: 20px;">
                            <input id="no_po" name="no_po" class="no_po form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{$dataPO->no_po}}" required/>
                        </div>
                        <div style="width: 20.8rem; margin-right: 20px;">
                            <input id="no_invoice" name="no_invoice" class="no_invoice form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{$dataPO->no_invoice}}" required/>
                        </div>
                        <div>
                            <input id="no_rab" style="width: 21.2rem;" name="no_rab" class="no_rab form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{$dataPO->no_rab}}" required/>
                        </div>
                </div>
                <div class="flex justify-between flex-col md:flex-row mb-3 mt-3">
                    ...
                    <input id="po_title" name="po_title" class="po_title form-input w-full md:w-3/4 px-2 py-1 read-only:bg-slate-200" type="text" value="{{$dataPO->po_title}}" required/>
                </div>
                <div class="flex justify-between flex-col md:flex-row">
                    <label class="text-sm font-medium mb-1"
                        ...
                        <select id="idsupplier" name="idsupplier"
                            ...
                            @foreach ( $supplier as $item )
                                <option value="{{$item->idsupplier}}" {{$dataPO->idsupplier == $item->idsupplier ? 'selected':''}}>{{$item->name}}</option>
                            @endforeach
                        </select>
                </div>
                <div class="flex justify-between flex-col md:flex-row mt-3">
                    <label class="block w-full md:w-1/4 text-sm font-medium mb-1" for="file">Upload Attachment PO<span
                        class="text-rose-500">*</span>&nbsp;: </label>
                    <input id="file" name="file" class="file form-input w-full md:w-3/4 px-2 py-1" type="file" accept="application/pdf"/>
                </div>
                <div class="flex justify-between flex-col md:flex-row">
                    ...
                    <label class="block w-full md:w-1/4 text-sm font-medium mt-1" for="payee_bank">Currency / Exchange Rate<span
                        ...
                        <div style="width: 31rem; margin-right: 41px;">
                            ...
                            <select id="currency" name="currency"
                                ...
                                <option value="{{$cur->currency}}" {{$dataPO->currency == $cur->currency ? 'selected':''}}>{{$cur->symbol}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <input id="crate" name="crate" style="width: 31.7rem;" class="crate form-input numeric-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{number_format($dataPO->crate, 0, ',', '.')}}" readonly required/>
                        </div>
                </div>
                <div class="flex flex-row mb-3 mt-3">
                    <label class="block w-full md:w-1/4 text-sm font-medium mt-1">Total / VAT / Grand Total<span class="text-rose-500">*</span></label>
                        <div style="width: 20.8rem; margin-right: 20px;">
                            <input id="total" name="total" class="total numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{number_format($dataPO->total, 0, ',', '.')}}" required/>
                        </div>
                        <div style="width: 20.8rem; margin-right: 20px;">
                            <input id="ppn" name="ppn" class="ppn numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200"  value="{{number_format($dataPO->ppn, 0, ',', '.')}}" type="text" required/>
                        </div>
                        <div>
                            <input id="gtotal" style="width: 21.2rem;" name="gtotal" class="gtotal numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{number_format($dataPO->gtotal, 0, ',', '.')}}" required readonly/>
                        </div>
                </div>
                <div class="flex flex-row mb-3 mt-3">
                    <label class="block w-full md:w-1/4 text-sm font-medium mt-1">WHT / Amount Due<span class="text-rose-500">*</span></label>
                        <div style="width: 31rem; margin-right: 41px;">
                            <input id="wht" name="wht" class="wht numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{number_format($dataPO->wht, 0, ',', '.')}}" required/>
                        </div>
                        <div>
                            <input id="amount_due" style="width: 31.7rem;" name="amount_due" class="amount_due numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{number_format($dataPO->amount_due, 0, ',', '.')}}" required readonly/>
                        </div>
                </div>
                ...
                <center><button class="btn bg-emerald-500 hover:bg-emerald-600 text-white mt-5" type="submit" id="create_offer">
                    <span class="xs:block ml-5 mr-5">Edit PO Pre System</span>
                </button> </center>
                @endif
            </form>
        </div>
            ...
            $.ajax({
                ...
                success: function(_response){
                    if (_response.st == '1') {
                        Swal.fire({
                            title: 'Success',
                            text: 'PO Pre System has been Updated',
                            icon: 'success',
                            ...
                            if (result.isConfirmed) {
                                urlRedirect = '/purchasing/po-presystem/edit';
