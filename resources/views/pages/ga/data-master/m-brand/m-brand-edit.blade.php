<x-app-layout>
                ...
                <div x-data="{ modalOpen: false }">
                    ...
                    <div id="feedback-modal"
                        ...
                        @keydown.escape.window="modalOpen = false">
                            ...
                            <form action="{{route('m-brand.create')}}" method="post">
                                @csrf
                                <!-- Modal content -->
                                <div class="px-5 py-4">
                                    <div class="text-sm">
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            ...
                                            <input id="brand" name="brand" type="text"
                                                class="brand form-input w-full px-2 py-1"
                                                required/>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#brand-table').on("click", ".btn-modal", function () {
                const id = $(this).data('id');
                const name = $(this).data('name');

                $.ajax({
                    type: "GET",
                    url: `/data-master/m-brand/getdata/${id}`,
                    success: function (response) {
                        const csrf_token = $('meta[name="csrf-token"]').attr('content');

                        $(".modal-content").html(`
                            <form method="post" class="type_update" enctype="multipart/form-data" action="/data-master/m-brand/update/${id}">
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-sm font-medium mb-1" for="brand1">Brand<span
                                                    class="text-rose-500">*</span></label>
                                            <input id="brand1" name="brand1" type="text"
                                                class="brand1 form-input w-full px-2 py-1" value="${name}"
                                                required/>
                                        </div>
                                    </div>
                                </div>
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            type: "DELETE",
                            url: `/data-master/m-brand/delete/${id}`,
                            success: function (response) {
