<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="flex flex-row">
            ...
            <form class="flex items-center mb-3" id="form-filter">
                ...
                <div class="relative ml-2 w-3/4 md:w-1/4">
                    <input id="form-search" class="form-input w-60 pl-9" type="number" placeholder="YYYY" min="1999"
            ...
            {{-- select sales --}}
            <label class="block text-sm font-medium text-lg mb-1 ml-3" for="salesname">Select Sales :</label>
                    <select class="salesname ml-3 mb-3 text-xs" id="salesname" name="salesname">
                        <option value="">All</option>
                        @foreach ($dataSales as $sales )
                        <option value="{{$sales->idsales}}">{{$sales->name}}</option>
                        @endforeach
                    </select>
        </div>
        ...
        <div class="flex flex-col col-span-full sm:col-span-6 bg-white shadow-sm rounded-sm border border-slate-200 mb-5">
            <div id="team-sales-chart-legend" class="px-5 py-3">
                <ul class="flex flex-wrap"></ul>
            </div>
            <div class="grow">
                <canvas id="team-sales-chart" width="595" height="248"></canvas>
            </div>
        </div>
        <div class="table-responsive">
            <table id="teamsales" class="table table-striped table-bordered" style="width:100%">
                <thead>
                    <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#form-filter").submit(function (e) {
                e.preventDefault();
                // teamSalesChart();
                $('#teamsales').DataTable().ajax.reload();
            });
            ...
            $('#teamsales').on("click", ".btn-modal", function () {
                ...
                const salesman = $(this).data("idsalesman");
                const salesname = $(this).data("salesname");

                $.ajax({
                    type: "GET",
                    url: `/sales/team-sales/getdetail/${year}/${monthNumber}/${salesman}`,
                    success: function (response) {
                        $(".modal-content").html(`
                            <div class="px-5">
                                ...
                                <div class="table-responsive mt-4">
                                    <table class="table table-striped table-bordered detail-team-sales"
                                        style="width:100%">
                                        <thead>
                                            <tr>
                        ...
                        $(".detail-team-sales").find('tbody').append(tableRow);
                    },
                });
            });
        });
