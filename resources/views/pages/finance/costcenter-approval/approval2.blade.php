<x-app-layout background="bg-white">
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <!-- Page header -->
        <div class="mb-8">
                <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Cost Center Approval 2 📝</h1>
        </div>
        ...
        @endif
        <form action="{{ route('reimburse-approvalga2.updatestatus', ['idRR' => $dataRR->idrec]) }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="grid md:grid-cols-3 gap-3">
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1" for="approve1_stat">User</label>
                    ...
                    <tbody class="tableProductAddBody" id="tableProductAddBody">
                        @foreach ($RRDetail as $item)
                            <tr>
                                ...
                                <td class="text-center">{{$dataRR->currency}}</td>
                        ...
                        <tr class="grandTotalRow">
                            <td class="text-center font-bold text-lg" colspan="5">Grand Total Reimburse</td>
                            <td class="text-right font-bold text-lg" id="grandTotal_text">{{number_format($dataRR->gtotal, 0, ',', '.')}}</td>
                            <td></td>
                        </tr>
                        @if ($dataRR->approved_total != null)
                        <tr class="grandTotalRow">
                            <td class="text-center font-bold text-lg" colspan="5">Reimburse Paid</td>
                            <td class="text-right font-bold text-lg">{{number_format($dataRR->approved_total, 0, ',', '.')}}</td>   
                            <td></td>
                        </tr>
                        @endif
                    </tbody>
