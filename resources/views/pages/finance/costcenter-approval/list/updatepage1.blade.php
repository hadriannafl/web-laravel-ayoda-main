    ...
    <script>
        ...
        for (const value of dataProducts) {
            ...
            function calculatePrice(iden) {
                ...
                const price5 = unitValue * forexValue;
                // const roundedPrice = Math.ceil(price5);
                $('#price_' + iden).val(allinDivider(price5));
            }
            ...
            function calculateVAT(iden) {
                ...
                const vatPercent = parseFloat($('#vatpercent_' + iden).val()) || 0;
                const totalVAT = subtotal * (vatPercent / 100); // Memperbarui dengan Math.floor()
                $('#totalvat_' + iden).val(allinDivider(totalVAT));
            }
            ...
            function calculateWHT(iden) {
                ...
                const whtPercent = parseFloat($('#wht_percent_' + iden).val()) || 0;
                const totalWHT = subtotal * (normaPercent / 100) * (whtPercent / 100); // Memperbarui dengan Math.floor()
                $('#total_wht_' + iden).val(allinDivider(totalWHT)); // Tidak perlu menggunakan toFixed() untuk mempertahankan angka bulat
            }
            ...
            function calculateGrandTotal(iden) {
                ...
                const grandTotal = parseFloat(total - totalWHT);
                const roundedGrandTotal = Math.round((grandTotal + Number.EPSILON) * 1000) / 1000;
                $('#gtotal_' + iden).val(allinDivider(roundedGrandTotal)); // Tidak perlu menggunakan toFixed() untuk mempertahankan angka bulat
            }
                        ...
                        modal_content += `<div class="modal-content text-xs px-5 py-4">
                                            ...
                                            <div class="flex justify-between flex-col md:flex-row mb-3">
                                                ...
                                                <input id="plate2_${iden}" name="plate2"
                                                    class="plate2 form-input w-full md:w-3/4 px-2 py-1 read-only:bg-slate-200"
                                                    type="text" value="${value.invoice_number === 'null' ? '' : (value.invoice_number || '')}"/>
                                            </div>
                                            <div class="flex justify-between flex-col md:flex-row mb-3">
                                                <label class="block w-full md:w-1/4 text-sm font-medium mb-1"
                                                    for="reimburse">Description
                                            ...
                                            <div class="flex flex-row mb-3">
                                                ...
                                                <div class="mb-3">
                                                    <input type="text" id="vatName_${iden}" name="vatName" style="width: 20rem; margin-left: 30px;" class="vatName form-input w-full px-2 py-1 ml-3" onchange="vatName(ths)" value="${value.vat === 'null' ? '' : (value.vat || '')}" hidden/>
                                                </div>
                                            ...
                                            <div class="flex flex-row mb-3">
                                                ...
                                                <div class="mb-3">
                                                    <input type="text" id="whtName_${iden}" name="whtName" style="width: 15rem; margin-left: 30px;" class="whtName w-full px-2 py-1 form-input ml-3" onchange="whtName(ths)" value="${value.wht === 'null' ? '' : (value.wht || '')}"/>
                                                </div>
            ...
            tableRow += `<tr id=row-${iden} class="text-xs">
                            ...
                            <td class="text-right"><span id="forex-text_${iden}">${allinDivider(value.forex)}</span><input type="text" name="forexs_${iden}" id="forexs_${iden}" value="${allinDivider(value.forex)}" hidden/><input type="text" name="currencys_${iden}" id="currencys_${iden}" value="${value.currency}" hidden/></td>
