<x-app-layout background="bg-white">
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <!-- Page header -->
        <div class="mb-8">
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Edit Payment Form 📝</h1>
        </div>

    <div class="px-5 py-4">
        <div class="space-y-3">
            <form method="post" enctype="multipart/form-data" action="{{ route('payment-list.update', ['payId' => $dataCPdetail->idrec]) }}" id="myForm">
                @csrf
                            ...
                            @click.outside="modalOpen = false" @keydown.escape.window="modalOpen = false">
                                ...
                                <div class="modal-content text-xs px-5 py-4">
                                    <div class="flex flex-row" hidden>
                                        ...
                                        <input id="id_company" name="id_company" style="width: 14rem;" class="id_company form-input w-full px-2 py-1 read-only:bg-slate-200" value="{{$dataCPdetail->compss}}" readonly/>
                                        ...
                                        <input id="beneficiary" name="beneficiary" style="width: 14rem;" class="beneficiary form-input w-full px-2 py-1 read-only:bg-slate-200" value="{{$dataCPdetail->beneficiary_acc}}" readonly/>
                                        <input id="formstypes" name="formstypes" style="width: 14rem;" class="formstypes form-input w-full px-2 py-1 read-only:bg-slate-200" value="{{$dataCPdetail->form_type}}" readonly/>
                                    </div>
                                    <div class="table-responsive">
                ...
                {{-- @if ($dataCPdetail->payment_type == 'WHT')
                    <div id="npwps">
                        <div class="flex flex-row mb-3 mt-3">
                            <label class="text-sm font-medium mt-1" for="npwp">NPWP ID / NPWP Name<span class="text-rose-500">*</span></label>
                                <div style="width: 31rem; margin-left: 11.6rem; margin-right: 41px;">
                                    <input id="npwp" name="npwp" class="npwp form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{$dataCP->npwp_id}}"/>
                                </div>
                                <div>
                                    <input id="npwp_name" name="npwp_name" style="width: 31.5rem;" class="npwp_name form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{$dataCP->npwp_name}}"/>
                                </div>
                        </div>
                        <div class="flex justify-between flex-col md:flex-row mt-3">
                            ...
                            <textarea id="npwp_address" name="npwp_address" class="npwp_address form-input w-full md:w-3/4 px-2 py-1 read-only:bg-slate-200" rows="3">{{$dataCP->npwp_address}}</textarea>
                        </div>
                    </div>
                @endif --}}
                <div class="flex justify-between flex-col md:flex-row mb-3">
                    ...
                    <input id="dates" name="dates" value="{{date('Y-m-d', strtotime($dataCPdetail->date))}}" class="dates selector form-input w-full md:w-3/4 px-2 py-1 read-only:bg-slate-200" data-date-format="YYYY/MM/DD" type="date" required/>
                </div>
                <div class="flex justify-between flex-col md:flex-row">
                    ...
                    <select id="company_id" name="company_id"
                        ...
                        <option value="{{$dataHeadCompany->id_company}}" {{$dataCPdetail->id_company == $dataHeadCompany->id_company ? 'selected':''}}>{{$dataHeadCompany->name}}</option>
                    </select>
                    @else
                ...
                <div class="flex flex-row mt-3">
                    <label class="text-sm font-medium mt-1" for="vatType">Payment Type / Currency / Amount/ Less Rounding<span class="text-rose-500">*</span></label>
                        ...
                        <div>
                            <input id="currency" name="currency" style="width: 14rem;" class="currency form-input w-full px-2 py-1 read-only:bg-slate-200" value="{{$dataCPdetail->currency}}" required readonly/>
                        </div>
                        <div>
                            <input id="amount" name="amount" style="width: 14rem; margin-left: 41px; margin-right: 30px;" class="amount numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{number_format($mathPayment->totals_amount, 0, ',', '.')}}" required/>
                        ...
                        <div>
                            <input id="lessRounding" name="lessRounding" style="width: 13rem; margin-left: 20px;" class="lessRounding extended-numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{number_format($dataCPdetail->rounding, 0, ',', '.')}}"/>
                ...
                @if (Auth::user()->company_id == '0' || Auth::user()->company_id == '999' || Auth::user()->company_id == '888')
                <div class="flex justify-between flex-col md:flex-row mt-3" id="payeesfrom">
                    ...
                    <select id="payfrom" name="payfrom" class="payfrom selector form-select w-full md:w-3/4 px-2 py-1">
                    ...
                    <div class="flex flex-row mb-3 mt-3">
                        <label class="block w-full md:w-1/4 text-sm font-medium mt-1" for="payee_bank">Bank Company / Bank CID / Bank Account<span
                            ...
                            <div style="width: 20.8rem; margin-right: 20px;">
                                <input id="payee_bank" name="payee_bank" class="payee_bank form-input w-full px-2 py-1 read-only:bg-slate-200" placeholder="Bank Company" value="{{$dataCPdetail->payee_bank}}" type="text" required readonly/>
                            </div>
                            <div style="width: 20.8rem; margin-right: 20px;">
                                <input id="companyID" name="companyID" class="companyID form-input w-full px-2 py-1 read-only:bg-slate-200" placeholder="Bank CID" value="{{$dataCPdetail->companyId}}" required readonly>
                            </div>
                            <div>
                                <input id="payee_acc" name="payee_acc" style="width: 21.2rem;" class="payee_acc form-input w-full px-2 py-1 read-only:bg-slate-200" placeholder="Bank Account" value="{{$dataCPdetail->payee_number}}" required readonly/>
                            </div>
                    </div>
                </div>
                <div class="flex justify-between flex-col md:flex-row mt-3">
                    ...
                    <input id="payment_date" name="payment_date" value="{{date('Y-m-d', strtotime($dataCPdetail->payment_date))}}"
                ...
                <div class="flex flex-row mb-3 mt-3">
                    <label class="text-sm font-medium mb-1 mt-1" for="vatType">Bank Charge / Duty Stamp Charge / Other Charge</label>
                        <div style="width: 20rem; margin-left: 1.1rem; margin-right: 41px;">
                            <input id="bank_charge" name="bank_charge" class="bank_charge numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" value="{{number_format($dataCPdetail->bank_charge, 0, ',', '.')}}" type="text"/>
                        </div>
                        <div>
                            <input id="duty_stamp_charge" name="duty_stamp_charge" style="width: 20rem;" class="duty_stamp_charge numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" value="{{number_format($dataCPdetail->duty_stamp_charge, 0, ',', '.')}}" type="text"/>
                        </div>
                        <div>
                            <input id="other_charge" name="other_charge" style="width: 20rem; margin-left: 41px;" class="other_charge numeric-input form-input w-full px-2 py-1 read-only:bg-slate-200" value="{{number_format($dataCPdetail->other_charge, 0, ',', '.')}}" type="text"/>
                        </div>
                </div>
                <div class="flex flex-row mb-3 mt-3">
                    <label class="block w-full md:w-1/4 text-sm font-medium mt-1" for="payee_bank">Approved By / Released By<span
                        ...
                        <div style="width: 31rem; margin-right: 41px;">
                            <input id="approved_by" name="approved_by" class="approved_by form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{$dataCPdetail->approved_by}}" placeholder="Approved By" required/>
                        </div>
                        <div>
                            <input id="released_by" name="released_by" style="width: 31.7rem;" class="released_by form-input w-full px-2 py-1 read-only:bg-slate-200" type="text" value="{{$dataCPdetail->released_by}}" placeholder="Released By" required/>
                        </div>
                </div>
                @if ($dataCPdetail->file_stat == null)
                    <center><button class="btn bg-emerald-500 hover:bg-emerald-600 text-white mt-5" type="submit" id="create_form">
                        <span class="xs:block ml-5 mr-5">Save Change</span>
                    </button> </center>
                @endif
            </form>
        </div>
...
@section('js-page')
<script>
    let subtotal = `{{number_format($mathPayment->totals_amount, 0, ',', '.')}}`;
    let approved = `{{number_format($approvespay, 0, ',', '.')}}`;
    ...
    $(document).ready(function () {
        ...
        $('#payment').on("click", ".btn-select", function () {
            var prods = [];
            let rowIdx = $('#tableProductAddBody tr').length-1;
            console.log(rowIdx);
            ...
            if (form_type == 'Cost Center') {
                var tr = `<tr id="row-${rowIdx}">
                            ...
                            <td class="text-right"><span id="totals-text_${rowIdx}">${divider(balance)}</span><input id="totals_${rowIdx}" name="rows[${rowIdx}][totals]" value="${newDivider2(balance)}" hidden/></td>
        ...
        $(".formtypes").on('change', function (e) {
            $('#payments').DataTable().ajax.reload();
        })
    ...
    const dataProducts = <?=$dataCPdetail1?>;
        ...
        for (const value of dataProducts) {
                ...
                if(value.id == item1.id){
                        modal_content += `<div class="modal-content text-xs px-5 py-4">
                                            <div class="flex justify-between flex-col md:flex-row mb-3">
                                                    ...
                                                    <textarea id="descs_${iden}" name="desc"
                                                        class="desc form-input w-full md:w-3/4 px-2 py-1 read-only:bg-slate-200"
                                                        type="text" maxlength="500" rows="3">${value.remarks}</textarea>
                                                </div>
                                            ...
                                            <div class="px-5 py-4 border-t border-slate-200">
                                                <div class="flex flex-wrap justify-end space-x-2">
                                                    ...
                                                    <button type="button"
                                                        class="btn btn-sm bg-indigo-500 hover:bg-indigo-600 text-white"
                                                        @click="modalOpen = false" onclick="updateDataProduct123('${iden}')">Update</button>
                                                </div>
                                            </div>
                                        </div>`
                }
            ...
            tableRow += `<tr id=row-${iden}>
                            <td><span id="companyName-text_${iden}">{{$dataCPdetail->companyName}}</span><input type="text" name="idrecss_${iden}" value="${value.idrec}" hidden/><input type="text" name="iden[]" value="${iden}" hidden/></td>
                            <td><span id="idcostpayment-text_${iden}">${value.id_costpayment}</span><input type="text" name="idcostpayment_${iden}" value="${value.id_costpayment}" hidden/><input type="text" name="date_${iden}" id="date_${iden}" value="${value.date}" hidden/><input type="text" name="paymentdate_${iden}" id="paymentdate_${iden}" value="${value.payment_date}" hidden/></td>
                            <td><input type="text" name="idcompany_${iden}" id="idcompany_${iden}" value="${value.id_company}" hidden/><input type="text" name="payytype_${iden}" id="payytype_${iden}" value="${value.payment_type}" hidden/><input type="text" name="kurs_${iden}" id="kurs_${iden}" value="${value.currency}" hidden/><input type="text" name="crates_${iden}" id="crates_${iden}" value="${value.crate}" hidden/><span id="plate-text_${iden}">{{$dataCPdetail->beneficiary_name}}</span></td>
                            <td><textarea name="remarksss_${iden}" id="remarksss_${iden}" hidden>${value.remarks}</textarea><input type="text" name="previouspay_${iden}" id="previouspay_${iden}" value="${value.previous_payment}" hidden/><span id="reimburse-text_${iden}">${value.remarks}</span><input type="text" name="aktifyn_${iden}" id="aktifyn_${iden}" value="${value.aktifyn}" hidden/></td>
                            <td class="text-right"><input type="text" name="amountPaid_${iden}" id="amountPaid_${iden}" value="${newDivider2(value.amount_paid)}" hidden/><span id="amountPaid-text_${iden}">${allinDivider(value.amount_paid)}</span><input type="text" name="createdat_${iden}" id="createdat_${iden}" value="${value.created_at}" hidden/><input type="text" name="createdby_${iden}" id="createdby_${iden}" value="${value.created_by}" hidden/></td>
                            <td class="flex flex-row justify-center" style="display: flex; justify-content: center;">
                <button type="button" onclick="deleteDataProduct123('${iden}', document.getElementById('amountPaid_${iden}').value, ${value.idrec})" class="btn btn-delete border-slate-200 hover:border-slate-300" > <svg class="w-4 h-4 fill-current text-rose-500 shrink-0" viewBox="0 0 16 16"> <path d="M5 7h2v6H5V7zm4 0h2v6H9V7zm3-6v2h4v2h-1v10c0 .6-.4 1-1 1H2c-.6 0-1-.4-1-1V5H0V3h4V1c0-.6.4-1 1-1h6c.6 0 1 .4 1 1zM6 2v1h4V2H6zm7 3H3v9h10V5z"/></svg></button>                
                    <div x-data="{ modalOpen: false }">
                                <button class="ml-2 btn bg-emerald-500 hover:bg-emerald-600 text-white" type="button"
                                @click.prevent="modalOpen = true" aria-controls="feedback-modal">
                                    <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 16 16"><title>pen</title><g stroke-width="1" stroke-linecap="round" fill="none" stroke="#ffff" stroke-miterlimit="10" class="nc-icon-wrapper" stroke-linejoin="round"><line x1="10" y1="3" x2="13" y2="6" data-cap="butt" stroke="#ffff"></line> <polygon points="12,1 15,4 5,14 1,15 2,11 " data-cap="butt"></polygon> </g></svg>
    ...
    var grandTotalRow = `<tr class="grandTotalRow bg-slate-400">
            ...
            <td class="text-right font-bold text-lg" id="grandTotal_text"><span id="grandTotal_text">{{number_format($mathPayment->totals_amount, 0, ',', '.')}}</span></td>
            <td></td>
        </tr>`;
    ...
    function deleteDataProduct123(positionTableRow, balance, approved_total, id_costpayment) {
        const positionTableRowVariable = positionTableRow
        subtotal -= balance;
        approved -= approved_total;
        $("#amount").val(divider(subtotal));
    ...
    function updateDataProduct123(iden) {
        var reimburse = $('#descs_'+iden).val();
        var reimburseTextarea = $(`#row-${iden}`).find(`textarea[name="remarksss_${iden}"]`);
        // Update the textarea value in the current row
    ...
    $(document).ready(function() {
        ...
        var id_company = '{{$dataCPdetail->id_company}}';
            if (userCompanyId == '0' || userCompanyId == '999' || userCompanyId == '888') {
                let options = `<option selected hidden value="" selected hidden>Select Company</option>`;
                if (id_company == '1') {
                    options = '<option selected hidden value="" selected hidden>Select Company</option><option value="{{$dataHeadCompany->id_company}}">{{$dataHeadCompany->name}}</option>';
                } else {
                    options += `<option value="{{$dataHeadCompany->id_company}}" {{$dataCPdetail->id_company == $dataHeadCompany->id_company ? 'selected':''}}>{{$dataHeadCompany->name}}</option><option value="{{$dataCPdetail->id_company}}" {{$dataCPdetail->id_company == $dataCPdetail->id_company ? 'selected':''}}>{{$dataCPdetail->companyName}}</option>`;   
                }
        ...
        if (userCompanyId == 0 || userCompanyId == 888 || userCompanyId == 999) {
            // Automatically select the company_id if only one option is available
            var selectedCompanyId = $('#company_id').val();
            
            if (selectedCompanyId) {
                // If company_id is preselected, filter options based on it
                filterPayFromOptions(selectedCompanyId);
            }

            // Enable company select field and allow user to change it
    ...
    $('#fullAmount').on('click', function () {
        $("#amount").val('{{number_format($mathPayment->totals_amount, 0, ',', '.')}}');        
    });
    ...
    $('#payment_type').on('change', function () {
        ...
        const currencyValue = '{{$dataCPdetail->currency}}';
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function checkAmountLimit() {
            ...
            var balance = parseFloat("{{$mathPayment->totals_amount}}");
            var balance1 = parseFloat("{{$mathPayment->totals_amount}}");
        ...
        $('#payment_type').on('change', function() {
            ...
            previousBalance = parseFloat("{{$mathPayment->totals_amount}}");
    ...
    //             if(_response.st == '1'){
    //                 window.location.href = '/finance/payment/list/payupdate';
