        ...
        validateDates();
         $(document).ready(function () {
            $('#payment').DataTable({
                ...
                ajax: {
                    url: "{{ route('payment-list.getdetail1') }}",
                    data:function(d){
                        d.from = $("#min").val()
                        d.to = $("#max").val()
            ...
            $('#payment').on("click", ".btn-print",  function () {
                ...
                Swal.fire({
                    ...
                    text: `Payment Voucher #${payments} will be Printed!`,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Yes, Print Payment Voucher!'
                }).then((result) => {
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            type: "POST",
                            url: `/finance/payment/list/printupdate/${id}`,
                            success: function (response) {
                                console.info("response: ", response)
                                const { status, message } = response;
                                if (status == 1) {
                                    urlRedirect = `/finance/payment/list/print/`+id;
                                    // window.open(urlRedirect, '_blank') = urlRedirect;
                                    window.open(urlRedirect, '_blank');
                                    Swal.fire({
                                        title: 'Success',
                                        text: 'Payment Has Been Printed',
                                        icon: 'success',
