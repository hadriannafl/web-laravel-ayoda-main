        ...
        validateDates();
         $(document).ready(function () {
            $('#payment').DataTable({
                ...
                ajax: {
                    url: "{{ route('payment-list.getdetail') }}",
                    data:function(d){
                        d.from = $("#min").val()
                        d.to = $("#max").val()
            ...
            $('#payment').on("click", ".btn-cancel",  function () {
                ...
                Swal.fire({
                    ...
                    text: `All Payment Detail with #${payments} will be Non Active!`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            type: "POST",
                            url: `/finance/payment/list/cancel/${id}`,
                            success: function (response) {
                                ...
                                if (status == 1) {
                                    ...
                                    // Update status directly if conditions are met
                                    // if (`${balance < total_paid}`) {
                                    //     // Perform another AJAX call to update status directly
                                    //     $.ajax({
                                    //         headers: {
                                    //             'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')
                                    //         },
                                    //         type: "POST",
                                    //         url: `/finance/payment/list/updatestatus/${id}`,
                                    //         success: function (response) {
                                    //             console.info("status update response: ", response)
                                    //             // Handle success or error response here
                                    //         },
                                    //         error: function (data) {
                                    //             console.info("status update error: ", data)
                                    //             // Handle error here
                                    //         }
                                    //     })
                                    // }else if (`${balance == total_paid}`) {
                                    //     // Perform another AJAX call to update status directly
                                    //     $.ajax({
                                    //         headers: {
                                    //             'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')
                                    ...
                                    Swal.fire({
                                        title: 'Error',
                                        text: 'Balance already default or Payment Already Canceled',
                                        icon: 'error',
                                        showCancelButton: false,
                                        confirmButtonColor: '#3085d6',
                                        confirmButtonText: 'OK',
