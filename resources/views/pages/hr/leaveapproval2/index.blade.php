<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <select id="approval2_status" class="approval2_status flex flex-row ml-3 mb-3 text-xs" name="approval2_status" hidden>
            <option value="Pending" selected>Pending</option>
        </select>
        <!-- Table -->
        <div class="table-responsive">
            <table id="approve2" class="table table-striped table-bordered text-xs" style="width:100%">
                <thead>
                    <tr>
                        <th></th>
    ...
    <script>
        $(document).ready(function () {
            $('#approve2').DataTable({
                ...
                ajax: {
                url: "{{ route('leaveapproval2.getdata') }}",
                    data:function(d){
                        d.approval2_status = $("#approval2_status").val()
                    }
                },
                columns: [
                    {
            ...
            $(".approval2_status").on('change', function (e) {
                e.preventDefault();
                $('#approve2').DataTable().ajax.reload();
            })

            $('#approve2').on("click", ".btn-modal", function () {
                ...
                $.ajax({
                    type: "GET",
                    url: `/hr/leaveapproval2/getdetail/${id}`,
            ...
            $('#approve2').on("click", ".btn-approve", function () {
                const id = $(this).data('id');
                const date = $(this).data('date');
                const from = $(this).data("from");
                const to = $(this).data("to");
                    ...
                    success: function (response) {
                        ...
                        <div class="px-5">
                                ...
                                <div class="grid md:grid-cols-3 gap-3">
                                    ...
                                    <form method="post" enctype="multipart/form-data" action="/hr/leaveapproval2/updateapprove/${id}">
                                        <input type="hidden" name="_token" value="${csrf_token}" />
                                        <div class="mt-3">
                                            <label class="block text-sm font-medium mb-1 text-left" for="approve2_notes">Approve 2 Notes</label>
                                            <textarea rows="4" id="approve2_notes" name="approve2_notes" class="approve2_notes form-input w-full px-2 py-1"
                                            type="text"></textarea>
                                        </div>
            ...
            $('#approve2').on("click", ".btn-denied", function () {
                const id = $(this).data('id');
                const date = $(this).data('date');
                const from = $(this).data("from");
                const to = $(this).data("to");
                    ...
                    success: function (response) {
                        ...
                        <div class="px-5">
                                ...
                                <div class="grid md:grid-cols-3 gap-3">
                                    ...
                                    <form method="post" enctype="multipart/form-data" action="/hr/leaveapproval2/updatedenied/${id}">
                                        <input type="hidden" name="_token" value="${csrf_token}" />
                                        <div class="mt-3">
                                            <label class="block text-sm font-medium mb-1 text-left" for="approve2_notes">Approve 2 Notes</label>
                                            <textarea rows="4" id="approve2_notes" name="approve2_notes" class="approve2_notes form-input w-full px-2 py-1"
