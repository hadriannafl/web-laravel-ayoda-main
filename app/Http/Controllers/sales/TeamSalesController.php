...
class TeamSalesController extends Controller
{
    ...
    {
        $dataSales = DB::table('salesmen')
            ->select('*'
            )->get();

        return view('pages.sales.team-sales.index', compact('dataSales'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $dataSalesGlobal = DB::table('orders')
            ...
            ->select('orders.idsalesman', 'salesmen.name as salesname')
            ->selectRaw("
        ...
        if ($request->input('salesname') != null) {
            $dataSalesGlobal = $dataSalesGlobal->where('orders.idsalesman', $request->input('salesname'));
        }
                ...
                ->addColumn('action', function ($dataSalesGlobal) {
                    return '<div x-data="{ modalOpen: false }">
                                <button class="btn btn-xs btn-modal bg-indigo-500 hover:bg-indigo-600 text-white" 
                                    ...
                                    data-idsalesman="' . $dataSalesGlobal->idsalesman . '" data-salesname="' . $dataSalesGlobal-> salesname. '">
                                    View
                                </button>
                                <!-- Modal backdrop -->
                                    ...
                                    <div class="bg-white rounded shadow-lg overflow-auto w-full max-h-full"
                                        ...
                                        <div class="px-5 py-3 border-b border-slate-200">
                                            <div class="flex justify-between items-center">
                                                <div class="font-semibold text-slate-800">Team Sales Detail</div>
    ...
    public function getDetail(Request $request, $year, $month, $param)
    {
        $dataDetailSalesGlobalQuery = DB::table('orders')
            ->leftJoin('companies', 'orders.company_id', 'companies.id')
            ->leftJoin('salesmen', 'orders.idsalesman', 'salesmen.idsales')
            ->select('orders.id', 'salesmen.idsales', 'orders.delivery_date', 'orders.inv_number', 'companies.name', 'salesmen.name as salesname', 'orders.dpp', 'orders.idsalesman');


        if ($request->input('salesname') != null) {
            $dataDetailSalesGlobalQuery = $dataDetailSalesGlobalQuery->where('orders.idsalesman', $request->salesname);
        }


        $dataDetailSalesGlobal = $dataDetailSalesGlobalQuery
            ->whereRaw("YEAR(delivery_date) = '$year' and MONTH(delivery_date) = '$month' and idsalesman='$param'")
