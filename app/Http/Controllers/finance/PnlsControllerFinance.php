...
class PnlsControllerFinance extends Controller
{
    public function index(){
        return view('pages.finance.pnls-finance.index');
    }
        ...
        if ($request->ajax()) {
            return DataTables::of($pnls)
                ...
                ->addColumn('action', function ($pnls) {
                    return '
                    <a href="/finance/pnls/file/' . $pnls->idrec . '" target="_blank" class="btn btn-xs bg-indigo-500 hover:bg-indigo-600 text-white">
                        View
                    </a>';
                })
                ->rawColumns(['action'])
