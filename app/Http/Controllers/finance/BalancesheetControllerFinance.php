...
class BalancesheetControllerFinance extends Controller
{
    public function index(){
        return view('pages.finance.balancesheet-finance.index');
    }
        ...
        if ($request->ajax()) {
            return DataTables::of($dataBs)
                ...
                ->addColumn('action', function ($dataBs) {
                    return '
                    <a href="/finance/balancesheet/file/' . $dataBs->idrec . '" target="_blank" class="btn btn-xs bg-indigo-500 hover:bg-indigo-600 text-white">
                        View
                    </a>';
                })
                ->rawColumns(['action'])
