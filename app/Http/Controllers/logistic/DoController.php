...
class DoController extends Controller
{
    ...
    {
        return view('pages.logistic.do.index');
    }
                ...
                ->addColumn('action', function ($dataDo) {
                    return ' <div x-data="{ modalOpen: false }">
                            <button  class="btn btn-sm btn-modal bg-indigo-500 hover:bg-indigo-600 text-white" 
                                ...
                                data-date = "' . $dataDo->delivery_date . '" data-address = "' . $dataDo->delivery_address . '" data-photo1="'.$dataDo->photo1.'" data-photo2="'.$dataDo->photo2.'">View</button>
    ...
    public function updateDO(Request $request, $id)
    {
        ...
        // $base64_photo1 = base64_encode($photo1);
        
        // photo2
        $file_photo2 = $request->file('photo2');    
        $photo2 = $file_photo2->openFile()->fread($file_photo2->getSize());
        // $base64_photo2 = base64_encode($photo2);
        
        $status = $request->input('status');

        // $statusNumber = $status == 'Deliver' ? '301' : '302'; 
    ...
    public function updatePage(Request $request, $code)
    {
        ...
        return view('pages.logistic.doupdate.doupdatepage', compact('viewDo'));
    }
    ...
    public function getDashboard(Request $request)
    {
        ...
        ->select(
            ...
            DB::raw("ISNULL(orders.photo1) as photo1"),
            DB::raw("ISNULL(orders.photo2) as photo2"),
            DB::raw("ISNULL(orders.invoice) as inv"),
            DB::raw("
                case
                    when orders.status = 1 then 'Shipping in Progress'
                    when orders.status = 2 then 'AWB / Shipping Information Uploaded'
                ...
                ->addColumn('action', function ($dataDo) {
                    return ' <div x-data="{ modalOpen: false }">
                            <button  class="btn btn-sm btn-modal bg-indigo-500 hover:bg-indigo-600 text-white" 
                                ...
                                data-inv="'.$dataDo->inv.'">View</button>
